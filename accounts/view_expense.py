...
def currentMonthreports(request,user):
    try:
        ...
        paymentData = Payment.objects.filter(payment_by = user,payment_date__month=current_month,payment_date__year=current_year).order_by('-payment_date')
...
def search_report(request,user):
    try:
        try:
            ...
            paymentData1 =Payment.objects.filter(payment_by = user ).filter(payment_for__icontains = request.GET.get("search",'').title())
            paymentData2 =Payment.objects.filter(payment_by = user ).filter(description__icontains= request.GET.get("search",''))
...
def deleteentry(request,id):
    try:
        ...
        entry.delete()
        messages.success(request,f'succesfully deleted entry {entry.amount} from {entry.payment_date.strftime("%d-%b-%Y")}')
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    except Exception as e:
        messages.error(request, f"An error occurred: will get back soon")
        return redirect('reports')
...
def undoDelEntries(request,id,user):
    try:
        ...
        entry.delete()
        messages.success(request,f'succesfully Undo entry {entry.amount} from {entry.payment_date.strftime("%d-%b-%Y")}')
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    except Exception as e:
        messages.error(request, f"An error occurred: will get back soon")
